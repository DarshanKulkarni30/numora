...
export const STRENGTH_ACTIONS: Record<string, StrengthAction> = {
  ...
  "You notice patterns early": {
    try: "write down one pattern you noticed, then rest",
    watch: "thinking until the month is gone",
  },
  ...
  "You can teach in plain words": {
    try: "help one person, then stop so you have fuel left",
...
function buildMeaning(
  ...
  if (weight === "core") {
    if (others.length) {
      return `This sits next to Life Path ${lifePath ?? ""}, and also shows in ${joinNames(others)}. It shows up a lot.`;
    }
    return `This sits next to Life Path ${lifePath ?? ""}. It shows up a lot.`;
  }
